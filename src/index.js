...
function deleteTextNodesRecursive(where) {
    ...
    let findElForRemove = (searchBase, acum) => {

        for (let node of searchBase.childNodes) {
            if (node.nodeType == 3) {
                acum.push({
                    where: searchBase,
                    what: node
                ...
                findElForRemove(node, acum);
            }
        }
    }

    findElForRemove(where, elForRemove);

    for (let i = 0; i < elForRemove.length; i++) {
        elForRemove[i].where.removeChild(elForRemove[i].what);
    }
...
function collectDOMStat(root) {
    ...
    function fn (startNode, accum) {
        
        for (let node of startNode.childNodes) {
            if (node.nodeType == 3) {
                accum.texts += 1;
            } else {
                accum.tags[node.tagName] = accum.tags[node.tagName] ? ++accum.tags[node.tagName] : 1;
                for (let cl of node.classList) {
                    accum.classes[cl] = accum.classes[cl] ? ++accum.classes[cl] : 1;
                }
                fn(node, accum);
            }
        }
...
function observeChildNodes(where, fn) {
    var observer = new MutationObserver (function(recs) {
            ...
            if (rec.addedNodes.length > 0) {
                param.type = 'insert';
                coln = rec.addedNodes;
            } else if (rec.removedNodes.length > 0) {
                param.type = 'remove';
                coln = rec.removedNodes;
            }
        
            for (let node of coln) {
